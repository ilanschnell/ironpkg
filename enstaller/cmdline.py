...
ENTHOUGHT_REPO = ("http://code.enthought.com/enstaller/eggs/%s/%s" %
    (PLAT, PLAT_VER))
ENTHOUGHT_UNSTABLE_REPO = ("http://code.enthought.com/enstaller/eggs/%s/%s/unstable" %
    (PLAT, PLAT_VER))
...
def install_requirement(requirements, target_repo=None, local_repos=None,
    ...
    for requirement in requirements:
        ...
        if interactive and len(packages) > 1:
            ...
            if selection == None:
                info("User selected no package for requirement %s" % requirement)
                continue
        ...
        while not response:
            ...
            for project, package in proposal.items():
                ...
                if active_project is None:
                    ...
                    print ("  Upgrade %s from %s to %s from %s" % (active_project.name,
            ...
            if not response:
                try:
                    ...
                    info("No proposed installation plan was acceptable to the user.")
                    return
                    ...
                    if interactive:
                        for dep in sorted(package.full_dependent_packages,
                            ...
                            print "Package %s depends on %s" % (dep.name,
                                package.name)
                        response = query_user("Remove '%s' from %s (y/n)? " % (package.name,
                            package.repository.location), default="y")
                        if not response:
                            continue
        ...
        for repo in repos:
            for package in repo.active_packages:
                if package.distribution in requirement:
                    deactivated = True
                    if interactive:
                        response = query_user("Deactivate '%s' from %s (y/n)? " % (package.name,
                            package.repository.location), default="y")
                        if not response:
                            continue                
