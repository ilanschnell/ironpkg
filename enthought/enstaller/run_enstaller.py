...
#
# The version of this file, used to verify that the enstaller egg is compatible
# with this launching script, since some of this code may be used elsewhere.
# The Enstaller egg must have a matching major.minor version (micro can change).
# ...the version must be a valid float!
#
RUN_ENSTALLER_VERSION = 1.0
RUN_ENSTALLER_REV = "$Rev$"[6:-2]
PYVER = "%s.%s" % (sys.version_info[0], sys.version_info[1])
VERSION_STRING = "\n%%prog - version %s rev. %s on Python %s\n" \
                 % (RUN_ENSTALLER_VERSION, RUN_ENSTALLER_REV, PYVER)
...
class Downloader( TextIO ) :
    ...
    from a repository.  (Written for use by the run_enstaller.py bootstrap
    script and expected to be extended by a separate Enstaller package).
    """
...
class EnstallerLauncher( TextIO ) :
    ...
    #
    # the Enstaller egg name pattern to match..."compat" is the egg which
    # matches this version of run_enstaller.py, the other is for checking for
    # new versions.
    # Note the $ at the end...needed to avoid matching things like foo.egg.info
    #
    compat_enstaller_egg_name = "enstaller-(%s\..*)-py%s.*\.egg$" \
                                % (RUN_ENSTALLER_VERSION, PYVER)
...
"""
    ...
    use_newer_enstaller_version_prompt = """
A newer version of Enstaller--possibly incompatible with this run_enstaller.py
script--was found.  The newer version may be compatible with this
run_enstaller.py file, but this cannot be guaranteed without downloading and
installing it.

The version known to be compatible with this run_enstaller.py script is:
%s

The newer version is:
%s

Would you like to download and install the newer version instead? (y/n) """

    enstaller_egg_not_found = """
An Enstaller egg for this run_enstaller.py and Python version was not found!
Use the --find-links option to specify a URL which has an Enstaller egg with
a version number of %s or greater for Python version %s
""" % (RUN_ENSTALLER_VERSION, PYVER)
...
"""
    ...
    def bootstrap_bootstrap( self ) :
        ...
        from enstaller.bootstrapper import Bootstrapper
    ...
    def get_enstaller_url( self ) :
        """
        Returns a URL to an Enstaller egg.  Both the highest *compatible*
        version as well as the highest version overall are found, and if
        prompting is True, the user has the option of installing the newer,
        possibly incompatible version instead.
        """
        #
        ...
        enstaller_url = self.downloader.find_latest_version(
            find_links, self.compat_enstaller_egg_name )

        if( enstaller_url is None ) :
            self.log( self.enstaller_egg_not_found )
        #
        # if the user is to be prompted, check for a new Enstaller version too
        #
        if( self.prompting ) :
            self.debug( "Looking for ALL eggs...\n" )
            highest_url = self.downloader.find_latest_version(
                find_links, self.enstaller_egg_name )
            #
            # if the highest version in all version != highest compat version,
            # then there must be a newer version available.
            #
            if( enstaller_url != highest_url ) :
                question = self.use_newer_enstaller_version_prompt \
                           % (enstaller_url or "<not found>", highest_url)
                if( self.prompt( question, False ) ) :
                    enstaller_url = highest_url
        #
    ...
    def run( self, argv, bootstrapping=False ) :
        ...
        try :
            ...
            import enstaller.api
            enstaller.api.RUN_ENSTALLER_VERSION = RUN_ENSTALLER_VERSION
            enstaller.api.RUN_ENSTALLER_REV = RUN_ENSTALLER_REV
        ...
        #
            ...
            # Enstaller egg and using its bootstrap module, then call
            # run_enstaller() again.
            #
    ...
    def _write_postmortem( self ) :
        ...
        header += "* run_enstaller: %s - version %s rev. %s\n" \
                  % (path.abspath( sys.argv[0] ), RUN_ENSTALLER_VERSION,
                     RUN_ENSTALLER_REV)
